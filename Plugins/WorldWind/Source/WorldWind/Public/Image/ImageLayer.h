...
#include "ImageLayer.generated.h"
...
/// TODO01: Update this code to take advantage of the Texture Manager
/// </summary>
/**
 * 
 */
UCLASS()
class WORLDWIND_API UImageLayer : public UObject
{
	GENERATED_BODY()
...
/*private:
		...
		// 不知道是啥
		/*World m_ParentWorld;
		Stream m_TextureStream = null;#1#

		bool _disableZbuffer;

		// 不知道是啥
		/*CustomVertex.PositionNormalTextured[] vertices;
		static CustomVertex.TransformedColored[] progressBarOutline = new CustomVertex.TransformedColored[5];
		static CustomVertex.TransformedColored[] progressBar = new CustomVertex.TransformedColored[4];#1#
		...
		int32 meshPointCount = 64;

		// 未完待续
		/*TerrainAccessor _terrainAccessor;#1#

		// 笔刷颜色？？？
		/*int32 progressBarBackColor = System.Drawing.Color.FromArgb(100, 255, 255, 255).ToArgb();
		int32 progressBarOutlineColor = System.Drawing.Color.SlateGray.ToArgb();
		int32 textColor = System.Drawing.Color.Black.ToArgb();#1#

		float downloadPercent;
		// 多线程的
		/*Thread downloadThread;#1#
	
		float verticalExaggeration;
		FString m_legendImagePath;
	
		/*Colorbar legendControl;#1#

		int32 m_TransparentColor = 0;


        bool m_renderGrayscale = false;

		FTimespan cacheExpiration = FTimespan(ETimespan::MaxTicks);

		// 定时器？？？
		/*System.Timers.Timer refreshTimer = null;#1#
		...
		/// </summary>
		public ImageLayer(FString name, float layerRadius ) : base (name)
		{
		...
		/// </summary>
		void Initialize(FDrawArgs drawArgs)
		{
			try
			{
		...
		/// </summary>
		void UpdateDownloadProgress(int32 bytesRead, int32 bytesTotal)
		{
		...
		/// </summary>
		void Update(FDrawArgs drawArgs)
		{
			try
			{
		...
		bool PerformSelectionAction(FDrawArgs drawArgs)
		{
			return false;
		}
		...
		// 不知道是干啥用的
		/*SurfaceImage m_SurfaceImage = null;#1#
		...
		protected virtual void CreateMesh()
		{
			...
			double latrange = FMath::Abs(maxLat - minLat);
			double lonrange;
			if(minLon < maxLon)
		...
		static FString getFilePathFromUrl(FString url)
		{
		...
		/// Switch to a different image
		/// </summary>
		void UpdateTexture(Stream textureStream)
		{
		...
		/// Switch to a different image
		/// </summary>
		void UpdateTexture(Stream textureStream, int transparentColor)
		{
			try
			{
				if(this.device != null)
		...
		void UpdateOpacity(float percent)
		{
